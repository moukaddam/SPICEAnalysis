...
//Fraction
//K-electron 975.65 keV :  0.036985967    // fraction of events 
//K-electron 481.69 keV :  0.0073863674    // fraction of events 
...
int     ReadInput(TString input_fname) ;
int     GrabHistos(TString fname) ;
...
void PeakSearch_207Bi_Simple(const char* fname = "his30091", const char* tag = "") {
   ...
   TFile *outFile ;
   //TString prefix =  "back_ground_subtracted_" ;
   TString prefix =  "pads_" ;
   TString outFileName = prefix + fname + ".root";
   outFile = new TFile(outFileName, "RECREATE");
   if (outFile->IsOpen() )  { //try a new directory
   cout << " Output root file is created : " << outFile <<endl ; 
   }
   else { 
   cout << " Output root file "<< outFile <<" could not be created, will continue..." <<endl ;
   }
   
   //***********************
   //read the ranges
   //***********************
	ReadInput("peaks_file.dat");
   ...
   //*********************** 
	...
	for (int odb_channel =  0; odb_channel < ODB_CHANNELS; odb_channel++) {
		...
		for (int i = 0 ; i < peaker[odb_channel].energy.size() ; i++) {
			...
			cCanvas[odb_channel]->cd(2)->Update() ;
			...
			//hist_bg[odb_channel]->GetXaxis()->SetRangeUser(mean-3*sigma,mean+3*sigma);				
			...
			cCanvas[odb_channel]->Update();
			
			outFile->cd() ;
			cCanvas[odb_channel]->Write();	
   ...
   //ofstream cal_file;
	...
	peaks_file	<< "\t"
				...
				<< "energy" <<"\t"
				<< fixed << showpoint << setw(8) << setprecision(2) 
				<< "energy_err" <<"\t"  // this value should be fixed in a proper way
				<< fixed << showpoint << setw(8) << setprecision(2) 
				<< "mean" <<"\t" 
				<< fixed << showpoint << setw(8) << setprecision(2) 
				<< "mean_err" <<"\t"
	...
	for (int odb_channel =  0; odb_channel < ODB_CHANNELS; odb_channel++) {	
		...
		for (int i = 0 ; i < peaker[odb_channel].energy.size() ; i++) {
			// write on disk 	
			//peaks_file << (odb_channel + odb_offset)<< "\t" << peaker[odb_channel].mnemonic << "\t" << peaker[odb_channel].address ;
			//peaks_file << "\t" << peaker[odb_channel].ring << "\t" << peaker[odb_channel].sector ; 		
		...
		//**************************	
		//peaks_file<< endl <<" This file was created using the Macro \
		/data1/moukaddam/SpiceTestSep2014/Calibration/SiLi/PeakSearch_207Bi_Simple.C "<<endl;
		peaks_file.close();
	
		//cal_file<< endl <<" This file was created using the Macro \
		/data1/moukaddam/SpiceTestSep2014/Calibration/SiLi/PeakSearch_207Bi_Simple.C "<<endl;
		//cal_file.close();
 ...
 int GrabHistos(TString fname) {
   ...
   inFile->SetName(path + inFileName);
	 ...
	 histos = (TFolder*)(inFile->FindObjectAny("histos"));
	 return success ;
   }  
...
int GetRingSector(int select, TString mnemonic ) {
	...
	//cout << sub << "   " << ringsector << "   sector:" << sector <<"   ring:" << ring << endl ; 
	//cin.get();
...
int   ReadInput(TString input_fname) {  
   ...
   cout << " Opening input file " << input_fname << endl ;  
   ifstream range_file ; 
   range_file.open (input_fname.Data()) ; 
...
void SubtractBackground(int odb_channel) {
	 ...
	 if (DEBUG) { cout << " SubtractBackground, channel " <<odb_channel<<endl; getchar(); }

		TString padtitle = Form("%d",odb_offset+odb_channel);
		cCanvas[odb_channel] = new TCanvas(padtitle,padtitle, 1000,700);
        cCanvas[odb_channel]->Divide(1,2) ;  
        
		// get the histogram, copy it and subtract the background 
		TString hname = Form("Chrg%d", odb_offset+odb_channel);
		hist[odb_channel] = (TH1F*)(histos->FindObjectAny(hname)); 
